...
y_pred_proba = gbm_classifier.predict_proba(X_test)  # Prediction probabilities for confidence level
...
train_accuracy = accuracy_score(y_train, gbm_classifier.predict(X_train)) * 100
...
conf_matrix_percent_df.to_csv('confusion_matrix_percent_gbm.csv', index=True)
metrics_df.to_csv('results_gbm.csv', index=False)

print("Accuracy:", test_accuracy)
print("RRSE:", rrse)
print("Confusion Matrix (in percentages) and metrics saved to 'confusion_matrix_percent_gbm.csv' and 'results_gbm.csv'")
...
plt.savefig('impact_of_training_gbm.png')
plt.close()
...
plt.legend()
plt.savefig('confidence_level_gbm.png')
plt.close()
...
plt.title('Confusion Matrix (in %) - GBM', fontsize=16, pad=20)
plt.tight_layout()
plt.savefig('confusion_matrix_percent_gbm.png')
plt.close()
...
plt.savefig('gbm_ft_importance.png', dpi=300)
plt.close()

print("GBM plots saved as 'impact_of_training_gbm.png', 'confidence_level_gbm.png', and 'confusion_matrix_percent_gbm.png'")



cv_scores = cross_val_score(gbm_classifier, X, y, cv=5)  # 5-fold cross-validation
